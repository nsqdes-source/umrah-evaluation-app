...
# الأعمدة التي يتوقعها محرك الحساب عند الاستيراد من Excel/CSV
REQUIRED_NUMERIC_FIELDS = [
    "total_entry_pilgrims", "luxury_pilgrims", "medium_pilgrims", "economy_pilgrims",
    "economy_gifts", "medium_gifts", "luxury_gifts", "umrah_plus_beneficiaries",
    "satisfaction_score_pct", "service_quality_pct", "total_complaints", "closed_complaints",
    "total_departing_pilgrims", "enrichment_beneficiaries", "total_visited_pilgrims",
    "unaffected_pilgrims", "total_entry_records", "matched_entry_records",
    "arrival_boarding_orders", "intercity_boarding_orders", "departure_boarding_orders",
    "total_exit_records", "matched_exit_records", "total_housing_programs", "confirmed_housing"
]
REQUIRED_BOOL_FIELDS = ["has_ministry_award", "has_severe_violation"]
...
def save_evaluation(company_name, results):
    """يحفظ التقييم في قاعدة البيانات. يعيد True عند النجاح، False عند الفشل."""
    try:
        conn = sqlite3.connect(DB_PATH)
        try:
            c = conn.cursor()
            ...
            ''', (
                ...
                results['penalties'],
                json.dumps(results['raw_data'], ensure_ascii=False, default=str)
            ))
            conn.commit()
            return True
        finally:
        ...
        st.error(f"⚠️ تعذر حفظ التقييم في قاعدة البيانات: {e}")
        return False
...
# ---------------------------------------------------------
def safe_ratio(numerator, denominator, scale, default_when_no_data=1.0):
    """
    نسبة موحّدة ومحمية من القسمة على صفر ومن تجاوز السقف.
    default_when_no_data: القيمة الافتراضية (0..1) المستخدمة عند عدم توفر بيانات (denominator=0)
    مثال: default_when_no_data=1.0 يعني "لا نعاقب الشركة على غياب البيانات"
          default_when_no_data=0.0 يعني "لا نمنحها علامة كاملة عند غياب البيانات"
    """
    if not denominator or denominator <= 0:
        ratio = default_when_no_data
    else:
...
def calculate_umrah_company_score(data):
    ...
    total_entry = data.get("total_entry_pilgrims", 0) or 0
    p_luxury = safe_ratio(data.get("luxury_pilgrims", 0) or 0, total_entry, 7.0, default_when_no_data=0.0)
    p_medium = safe_ratio(data.get("medium_pilgrims", 0) or 0, total_entry, 5.0, default_when_no_data=0.0)
    p_economy = safe_ratio(data.get("economy_pilgrims", 0) or 0, total_entry, 3.0, default_when_no_data=0.0)
    score_packages = p_luxury + p_medium + p_economy

    # ب. تجربة المعتمر وجودة الخدمة والمخالفات التشغيلية (45%)
    p_satisfaction = max(0.0, min(1.0, (data.get("satisfaction_score_pct", 0.0) or 0.0) / 100.0)) * 10.0
    p_quality = max(0.0, min(1.0, (data.get("service_quality_pct", 0.0) or 0.0) / 100.0)) * 5.0

    total_complaints = data.get("total_complaints", 0) or 0
    closed_complaints = data.get("closed_complaints", 0) or 0
    # لا توجد بلاغات = لا يوجد ما يُعاقب أو يُكافأ عليه صراحة، فنمنحها العلامة الكاملة (سياسة: غياب المشاكل ≠ إخفاق)
    p_complaints = safe_ratio(closed_complaints, total_complaints, 10.0, default_when_no_data=1.0)

    total_departing = data.get("total_departing_pilgrims", 0) or 0
    p_enrichment = safe_ratio(data.get("enrichment_beneficiaries", 0) or 0, total_departing, 10.0, default_when_no_data=0.0)
    ...
    # لم تتم زيارتها ميدانياً بعد = لا يوجد دليل على مخالفة، لذا نعتبرها محايدة (علامة كاملة) حتى تُفحص لاحقاً
    p_compliance = safe_ratio(unaffected, total_visited, 10.0, default_when_no_data=1.0)

    score_exp = p_satisfaction + p_quality + p_complaints + p_enrichment + p_compliance

    # ج. الالتزام بالبرنامج (40%)
    total_entry_rec = data.get("total_entry_records", 0) or 0
    # لا توجد سجلات قدوم = لا يمكن إثبات الالتزام، فالافتراضي صفر (متوافق مع بقية مؤشرات هذا المحور)
    p_entry_match = safe_ratio(data.get("matched_entry_records", 0) or 0, total_entry_rec, 10.0, default_when_no_data=0.0)

    p_arr_boarding = safe_ratio(data.get("arrival_boarding_orders", 0) or 0, total_entry, 5.0, default_when_no_data=0.0)
    p_inter_boarding = safe_ratio(data.get("intercity_boarding_orders", 0) or 0, total_departing, 5.0, default_when_no_data=0.0)
    p_dep_boarding = safe_ratio(data.get("departure_boarding_orders", 0) or 0, total_departing, 5.0, default_when_no_data=0.0)

    total_exit_rec = data.get("total_exit_records", 0) or 0
    p_exit_match = safe_ratio(data.get("matched_exit_records", 0) or 0, total_exit_rec, 10.0, default_when_no_data=0.0)

    total_housing = data.get("total_housing_programs", 0) or 0
    p_housing = safe_ratio(data.get("confirmed_housing", 0) or 0, total_housing, 5.0, default_when_no_data=0.0)
    ...
    gift_incentive = min(5.0, max(0.0, (gift_points / 8000.0) * 5.0))

    umrah_plus_points = (data.get("umrah_plus_beneficiaries", 0) or 0) * 2
    umrah_plus_incentive = min(10.0, max(0.0, (umrah_plus_points / 2000.0) * 10.0))
...
def validate_input_data(data):
    """يرجع قائمة تحذيرات لعلاقات منطقية غير متسقة في البيانات (لا توقف الحساب لكن تُنبّه المستخدم)."""
    warnings = []
    checks = [
        ("closed_complaints", "total_complaints", "عدد البلاغات المعالجة أكبر من إجمالي البلاغات"),
        ("unaffected_pilgrims", "total_visited_pilgrims", "عدد المعتمرين غير المتأثرين أكبر من إجمالي من تمت زيارتهم"),
        ("matched_entry_records", "total_entry_records", "سجلات القدوم المتطابقة أكبر من إجمالي سجلات القدوم"),
        ("matched_exit_records", "total_exit_records", "سجلات المغادرة المتطابقة أكبر من إجمالي سجلات المغادرة"),
        ("confirmed_housing", "total_housing_programs", "عدد المؤكد سكنهم أكبر من إجمالي برامج السكن"),
    ]
    for part_key, whole_key, msg in checks:
        part = data.get(part_key, 0) or 0
        whole = data.get(whole_key, 0) or 0
        try:
            if float(part) > float(whole):
                warnings.append(f"⚠️ {msg} ({part} > {whole}).")
        except (TypeError, ValueError):
            warnings.append(f"⚠️ قيمة غير رقمية في الحقلين المرتبطين بـ «{msg}».")
    return warnings


def normalize_imported_row(raw_row: dict):
    """
    يحوّل صفاً مستورداً من Excel/CSV إلى القاموس الذي يتوقعه محرك الحساب:
    - يحوّل أنواع numpy إلى أنواع بايثون الأصلية (قابلة للتسلسل عبر json.dumps)
    - يتحقق من وجود الأعمدة المطلوبة ويُبلغ بالمفقود منها
    - يفسّر الحقول المنطقية (True/False) من نص أو رقم بشكل صريح بدل الاعتماد على bool(str)
    """
    def to_native(v):
    ...
    def to_bool(v):
        if isinstance(v, str):
            return v.strip().lower() in ("true", "1", "yes", "نعم", "صح", "y")
        try:
            return bool(int(v))
        except (TypeError, ValueError):
            return bool(v)

    clean = {k: to_native(v) for k, v in raw_row.items()}

    missing_numeric = [f for f in REQUIRED_NUMERIC_FIELDS if f not in clean]
    missing_bool = [f for f in REQUIRED_BOOL_FIELDS if f not in clean]

    data = {}
    for f in REQUIRED_NUMERIC_FIELDS:
        try:
            data[f] = float(clean.get(f, 0) or 0)
        except (TypeError, ValueError):
            data[f] = 0
    for f in REQUIRED_BOOL_FIELDS:
        data[f] = to_bool(clean.get(f, False))

    return data, missing_numeric, missing_bool
...
def generate_ai_advisor_report(results, api_key=None, language="العربية"):
    ...
    if (raw.get('umrah_plus_beneficiaries', 0) or 0) < 1000:
        pts_needed = 1000 - (raw.get('umrah_plus_beneficiaries', 0) or 0)
...
CRITICAL INSTRUCTION: The full analysis and all recommendations MUST be strictly in English."""
            ...
            if language == "العربية":
                ...
                st.warning("⚠️ Could not get a response from Gemini models. Showing the automatic base report instead.")
            with st.expander("تفاصيل الخطأ (للمطورين)" if language == "العربية" else "Error details (for developers)"):
...
with tab1:
    ...
    if input_mode == "إدخال يدوي (Manual)":
        ...
        with col2:
            ...
            if closed_complaints > total_complaints:
                st.error("⚠️ عدد البلاغات المعالجة أكبر من إجمالي البلاغات الواردة.")
            ...
            if unaffected_pilgrims > total_visited_pilgrims:
                st.error("⚠️ عدد المعتمرين غير المتأثرين أكبر من إجمالي من تمت زيارتهم.")
                ready_to_calculate = False
        ...
        with col3:
            ...
            if matched_entry_records > total_entry_records:
                st.error("⚠️ سجلات القدوم المتطابقة أكبر من إجمالي سجلات القدوم.")
                ready_to_calculate = False
            ...
            if matched_exit_records > total_exit_records:
                st.error("⚠️ سجلات المغادرة المتطابقة أكبر من إجمالي سجلات المغادرة.")
                ready_to_calculate = False
            ...
            if confirmed_housing > total_housing_programs:
                st.error("⚠️ عدد المؤكد سكنهم أكبر من إجمالي برامج السكن.")
                ready_to_calculate = False

    else:
        st.info(
            "الأعمدة المطلوبة في ملف Excel/CSV:\n\n"
            + "، ".join(REQUIRED_NUMERIC_FIELDS + REQUIRED_BOOL_FIELDS)
        )
        ...
        if uploaded_file:
            ...
            if df.empty:
                st.error("⚠️ الملف لا يحتوي على أي صفوف بيانات.")
                st.stop()

            if len(df) > 1:
                st.warning(
                    f"⚠️ الملف يحتوي على {len(df)} صفوف. سيتم استخدام **الصف الأول فقط** كتقييم لهذه الشركة. "
                    "لتقييم عدة شركات، ارفع كل شركة في ملف مستقل أو استخدم صفاً واحداً في كل مرة."
                )

            data, missing_numeric, missing_bool = normalize_imported_row(df.iloc[0].to_dict())

            if missing_numeric or missing_bool:
                st.error(
                    "⚠️ الأعمدة التالية غير موجودة في الملف وسيتم اعتبارها صفر/False:\n\n"
                    + "، ".join(missing_numeric + missing_bool)
                )

            val_warnings = validate_input_data(data)
    ...
    if st.button("🚀 احتساب التقييم وحفظ النتيجة", type="primary", disabled=not ready_to_calculate):
        if input_mode == "إدخال يدوي (Manual)":
            data = {
        ...
        saved = save_evaluation(company_name, results)
    ...
    if 'latest_results' in st.session_state and st.session_state['latest_results']:
        ...
        try:
            ...
            safe_name = re.sub(r'[\\/*?:"<>|]', "_", comp_name).strip() or "شركة"
            st.download_button(
                label="📄 تصدير التقرير النهائي (PDF)",
                data=pdf_bytes,
                file_name=f"تقرير_تقييم_{safe_name}.pdf",
                mime="application/pdf",
                width="stretch"
            )
        except Exception as e:
            st.error(f"⚠️ تعذر توليد ملف الـ PDF: {e}")
    ...
    try:
        ...
        try:
            df_history = pd.read_sql_query(
                "SELECT id, eval_date, company_name, final_score, tier, score_packages, score_exp, score_prog, incentives, penalties FROM evaluations ORDER BY id DESC",
                conn
            )
        finally:
            conn.close()
    except sqlite3.Error as e:
        st.error(f"⚠️ تعذر تحميل السجل التاريخي: {e}")
